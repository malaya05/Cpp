...
// moveToEnd SLL
void AnyList::moveToEnd()
{
		Node* firstNode = ptrToFirst;
		Node* curr = ptrToFirst;
		while (curr->getPtrToNext() != nullptr)
			curr = curr->getPtrToNext();
		ptrToFirst = ptrToFirst->getPtrToNext();
		curr->setPtrToNext(firstNode);
		firstNode->setPtrToNext(nullptr);
}

// printReverse
void AnyList::printReverse() const
{
	int n = count;
	for (int coun= count; coun >= 1; coun--)
	{
		Node *curr = ptrToFirst;
		while ( n!=1)
		{		
